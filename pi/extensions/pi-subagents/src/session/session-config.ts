...
export interface SessionConfig {
  ...
  systemPrompt: string;
  /** Built-in tool name allowlist for this agent type. undefined = no restriction. */
  toolNames?: string[];
  /** Agent 配置中指定的内置工具名称。undefined = 使用全部内置工具（如 general-purpose）。
   *  用于 applyRecursionGuard：仅过滤内置工具，保留全部扩展工具。 */
...
export function assembleSessionConfig(
  ...
  const builtinToolNames = agentConfig.builtinToolNames;
