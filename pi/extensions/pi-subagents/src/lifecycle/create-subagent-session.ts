...
/**
 * Apply the recursion guard and expand tool set to include extension-registered
 * tools. Runs after `bindExtensions` so extension tools are registered.
 *
 * 1. Get ALL tools from session.getAllTools() (built-in + extension-registered)
 * 2. Remove recursion-guarded dispatch tools (subagent, get_subagent_result,
 *    steer_subagent)
 * 3. If the agent config restricts built-in tools (e.g. Explore/Plan are
 *    read-only), filter only built-in tools to the allowed subset — keep all
 *    extension-registered tools.
 * 4. Set the result as active tools.
 */
function applyRecursionGuard(
  session: AgentSession,
  builtinToolNames?: string[],
): void {
  // Get all available tools (built-in + extension-registered after bindExtensions)
  const allToolNames = session.getAllTools().map((t) => t.name);

  // 1. Remove recursion-guarded dispatch tools
  let filtered = allToolNames.filter(
    (t) => !EXCLUDED_TOOL_NAMES.includes(t),
  );

  // 2. If agent config restricts built-in tools, filter only built-in tools
  //    — keep all extension-registered tools
  if (builtinToolNames?.length) {
    const builtinSet = new Set(BUILTIN_TOOL_NAMES);
    const allowedBuiltins = new Set(builtinToolNames);
    filtered = filtered.filter(
      (t) => !builtinSet.has(t) || allowedBuiltins.has(t),
    );
  }
...
export async function createSubagentSession(
  ...
  const { session } = await deps.io.createSession({
    ...
    modelRegistry: snapshot.modelRegistry,
    model: cfg.model,
    ...(cfg.toolNames?.length ? { tools: cfg.toolNames } : {}),
  ...
  try {
    ...
    applyRecursionGuard(session, cfg.builtinToolNames);
  } catch (err) {
